...
interpolatePoint(const trajectory_msgs::msg::JointTrajectoryPoint &p1,
  ...
  if (!p1.accelerations.empty() && !p2.accelerations.empty()) {
    ...
    for (size_t i = 0; i < p1.accelerations.size(); ++i) {
      result.accelerations[i] =
          p1.accelerations[i] +
          alpha * (p2.accelerations[i] - p1.accelerations[i]);
    }
  }
...
// needed
trajectory_msgs::msg::JointTrajectoryPoint
sampleTrajectoryAtTime(const trajectory_msgs::msg::JointTrajectory &traj,
                       double t) {
...
public:
  DualRobotMoveitExample()
      : MoveitExample("moveit_dual_robot_planning_example",
                      "dual_manipulator") {}
  ...
  bool run() {
    ...
    if (!planAndExecutePerArmCartesianPath()) {
      RCLCPP_ERROR(this->get_logger(),
                   "per-arm Cartesian planning/execution failed, aborting");
      return false;
    }
    RCLCPP_INFO(
        this->get_logger(),
        "per-arm Cartesian step completed, continuing with joint motion");
...
private:
  bool planAndExecuteJointMotion() {
    std::vector<double> joint_goal(kDualHomeJointPositions.begin(),
                                   kDualHomeJointPositions.end());

    joint_goal[kRobot1Joint1Index] += -0.15;
    joint_goal[kRobot2Joint1Index] += 0.15;

    auto trajectory =
        planToPosition(joint_goal, "ompl", "RRTConnectkConfigDefault");
    if (!trajectory) {
      RCLCPP_ERROR(this->get_logger(), "planning failed for group '%s'",
                   planningGroup().c_str());
      return false;
    }

    RCLCPP_INFO(this->get_logger(),
                "planned dual-arm trajectory with group '%s'",
                planningGroup().c_str());

    if (!executeTrajectory(*trajectory)) {
      RCLCPP_ERROR(this->get_logger(), "execution failed for group '%s'",
                   planningGroup().c_str());
      return false;
    }

    RCLCPP_INFO(this->get_logger(),
                "executed dual-arm trajectory with group '%s'",
                planningGroup().c_str());
    return true;
  }
  ...
  bool planAndExecutePerArmCartesianPath() {
    ...
    const auto robot1_current =
        robot1_group.getCurrentPose("robot1_tool0").pose;
    const auto robot2_current =
        robot2_group.getCurrentPose("robot2_tool0").pose;
    ...
    const bool robot1_planned =
        static_cast<bool>(robot1_group.plan(robot1_plan));
    const bool robot2_planned =
        static_cast<bool>(robot2_group.plan(robot2_plan));
    ...
    combined_jt.joint_names.insert(combined_jt.joint_names.end(),
    ...
    for (double t : time_points) {
      ...
      combined_pt.positions.insert(combined_pt.positions.end(),
                                   pt2.positions.begin(), pt2.positions.end());

      // Combine velocities if available
      if (!pt1.velocities.empty() && !pt2.velocities.empty()) {
        combined_pt.velocities = pt1.velocities;
        combined_pt.velocities.insert(combined_pt.velocities.end(),
                                      pt2.velocities.begin(),
                                      pt2.velocities.end());
      } else {
      ...
      if (!pt1.accelerations.empty() && !pt2.accelerations.empty()) {
        ...
        combined_pt.accelerations.insert(combined_pt.accelerations.end(),
                                         pt2.accelerations.begin(),
                                         pt2.accelerations.end());
      } else {
    ...
    RCLCPP_INFO(this->get_logger(), "planned and executed synchronized "
...
int main(int argc, char *argv[]) {
  ...
  if (success) {
    RCLCPP_INFO(node->get_logger(), "Successfully planned and executed one "
                                    "combined trajectory for both robots.");
  } else {
    RCLCPP_ERROR(node->get_logger(),
                 "Combined dual-robot planning or execution failed.");
  }
