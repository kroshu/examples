...
public:
  ...
  moveit_msgs::msg::RobotTrajectory::SharedPtr drawCircle() {
    ...
    double fraction = move_group_interface_->computeCartesianPath(
        waypoints, 0.005, trajectory);
    RCLCPP_INFO(LOGGER, "Planning done!");

    if (fraction < 1) {
      RCLCPP_ERROR(LOGGER,
                   "Could not compute trajectory through all waypoints!");
      return nullptr;
    } else {
  ...
  void addCollisionBox(const geometry_msgs::msg::Vector3 &position,
  ...
  void
  setOrientationConstraint(const geometry_msgs::msg::Quaternion &orientation) {
    moveit_msgs::msg::OrientationConstraint orientation_constraint;
    moveit_msgs::msg::Constraints constraints;
    orientation_constraint.header.frame_id =
        move_group_interface_->getPlanningFrame();
    orientation_constraint.link_name =
        move_group_interface_->getEndEffectorLink();
  ...
  void drawTitle(const std::string &text) {
    ...
    moveit_visual_tools_->publishText(text_pose, text, rviz_visual_tools::RED,
