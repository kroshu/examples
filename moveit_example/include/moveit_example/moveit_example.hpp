...
#ifndef MOVEIT_EXAMPLE__MOVEIT_EXAMPLE_HPP_
#define MOVEIT_EXAMPLE__MOVEIT_EXAMPLE_HPP_
...
public:
    ...
    const std::vector<MotionSegment> & motion_sequence)
  {
    ...
    for (size_t index = 0; index < motion_sequence.size(); ++index)
    {
      ...
      move_group_interface_->setPlannerId(motion_sequence[index].planner_id);
      move_group_interface_->setPoseTarget(motion_sequence[index].pose);
      ...
      motion_segment.blend_radius =
        index == motion_sequence.size() - 1 ? 0.0 : motion_sequence[index].blend_radius;
    ...
    if (!response || response->response.planned_trajectories.empty())
    {
    ...
    return std::make_shared<moveit_msgs::msg::RobotTrajectory>(
      response->response.planned_trajectories.front());
  }
